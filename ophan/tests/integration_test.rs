...
use ophan::gateway::{AppContext, CompiledRoute, build_app_context};
...
fn oauth_complete_flow() {
    ...
    use ophan::middlewares::auth::AuthMiddleware;
...
"#,
    ...
    let config = ophan::config::OphanConfig::parse().expect("parse config");
    let ctx = ophan::gateway::build_app_context(&config).expect("build app context");

    eprintln!("Parsed: {} routes, {} upstreams", config.routes.len(), config.upstreams.len());

    let r = ctx.router.find_route(Some("api.example.me"), "GET", "/v1");
    match &r {
    ...
    let r2 = ctx.router.find_route(Some("api.example.me"), "GET", "/v1/realtime/sse");
    match &r2 {
    ...
    assert!(r.is_ok(), "Route should match /v1 for api.example.me");
}
...
fn debug_catch_all_with_prefix() {
    ...
    eprintln!("=== Testing /v1 with host api.example.me ===");
    let result = ctx.router.find_route(Some("api.example.me"), "GET", "/v1");
    match &result {
    ...
    let result3 = ctx.router.find_route(Some("api.example.me"), "GET", "/v1/realtime/sse");
    match &result3 {
        Ok(m) => eprintln!("✅ MATCH: backend={:?}", m.value.backend),
        Err(e) => eprintln!("❌ ERROR: {:?}", e),
    }
    ...
    if result.is_err() {
        ...
        let r = ctx2.router.find_route(Some("api.example.me"), "GET", "/v1");
        match &r {
