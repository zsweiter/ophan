...
impl ListenerConfig {
    ...
    pub fn https(name: impl Into<String>, addr: SocketAddr, cert: SSLConfig) -> Self {
        Self {
            ...
            protocols: vec![
                NetworkProtocol::Http1 { allow_websocket_upgrade: false }, // Para HTTPS, deshabilitamos WebSocket Upgrade por seguridad (Evita ataques de WebSocket hijacking sobre TLS)
...
// Implementamos Default para CORS con una configuración segura por defecto (Sin orígenes permitidos, métodos limitados, sin credenciales)
// Esto obliga a configurar explícitamente CORS para evitar configuraciones inseguras por defecto (Principio de seguridad por defecto)
