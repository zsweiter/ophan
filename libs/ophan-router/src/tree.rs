...
use super::escape::{UnescapedRef, UnescapedRoute};
...
impl<'node, T> InsertState<'node, T> {
    ...
    fn set_child(self, i: usize) -> InsertState<'node, T> {
        match self.child {
            None => InsertState { parent: self.parent, child: Some(i) },
            Some(prev) => InsertState { parent: &mut self.parent.children[prev], child: Some(i) },
        }
    }
}
...
impl<T> Node<T> {
    ...
    pub fn insert(&mut self, route: String, val: T) -> Result<(), InsertError> {
        ...
        let mut state = InsertState { parent: self, child: None };
        ...
        'walk: loop {
            ...
            if let NodeType::Param { suffix: has_suffix } = state.node().node_type {
                ...
                if remaining[0] != b'{' || remaining.is_escaped(0) {
                    let child = state.node_mut().add_child(Node { node_type: NodeType::Static, priority: 1, ..Node::default() });
            ...
            if state.node().wild_child {
                ...
                if let Some(parent) = state.parent() {
                    ...
                    if !parent.prefix.ends_with(b"/") && matches!(state.node().node_type, NodeType::Param { suffix: true }) {
                        let terminator = remaining.iter().position(|&b| b == b'/').map(|b| b + 1).unwrap_or(remaining.len());
    ...
    fn insert_route(&mut self, mut prefix: UnescapedRef<'_>, val: T) -> Result<&mut Node<T>, InsertError> {
        ...
        loop {
            ...
            let terminator = prefix.iter().position(|&b| b == b'/').map(|b| b + 1).unwrap_or(prefix.len());
            ...
            if prefix[0] != b'{' || prefix.is_escaped(0) {
                ...
                let child = node.add_child(Node { priority: 1, ..Node::default() });
                node = &mut node.children[child];
            }
        }
    }
    ...
    fn add_suffix_child(&mut self, child: Node<T>) -> usize {
        let i = self.children.partition_point(|node| node.prefix.len() >= child.prefix.len());
            ...
            'walk: loop {
                ...
                if !backtracking {
                    ...
                    if let Some(i) = node.indices.iter().position(|&c| c == next) {
                        ...
                        // match.
                        if node.wild_child {
                            skipped.push(Skipped { node, path: previous, params: params.len() });
                        }
    ...
    T: fmt::Debug,
{
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        #[cfg(test)]
        {
            ...
            let params = self.remapping.iter().map(|x| std::str::from_utf8(x).unwrap()).collect::<Vec<_>>();
