...
impl<'k, 'v> Param<'k, 'v> {
    const EMPTY: Param<'static, 'static> = Param { key: b"", value: b"" };
...
impl<'k, 'v> fmt::Debug for Param<'k, 'v> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("Param").field("key", &self.key_str()).field("value", &self.value_str()).finish()
    }
}
...
impl<'k, 'v> Params<'k, 'v> {
    ...
    #[inline]
    pub fn new() -> Self {
        Self { kind: ParamsKind::Small([Param::EMPTY; SMALL], 0) }
    }
    ...
    pub fn get(&self, key: impl AsRef<str>) -> Option<&'v str> {
        ...
        match &self.kind {
            ParamsKind::Small(arr, len) => arr.iter().take(*len).find(|param| param.key == key).map(Param::value_str),
            ParamsKind::Large(vec) => vec.iter().find(|param| param.key == key).map(Param::value_str),
        }
    }
...
impl<'k, 'v> Iterator for ParamsIter<'_, 'k, 'v> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match self.kind {
            ParamsIterKind::Small(ref mut iter) => iter.next().map(|p| (p.key_str(), p.value_str())),
...
mod tests {
    ...
    #[test]
    fn heap_alloc() {
        let vec = vec![("hello", "hello"), ("world", "world"), ("foo", "foo"), ("bar", "bar"), ("baz", "baz")];
